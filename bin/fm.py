...
class FileComment:
    ...
    def __repr__(self):
        ...
        return '\n'.join(textwrap.TextWrapper(
            ...
            ).wrap('%-9s %s' % (self.repname, self.comment)))
    ...
    def update(self, inc):
        ...
        try:
            comment = raw_input('%s: ' % self.repname)
        ...
        if len(comment) == 0:  # pass to next
            ...
            print self.hist() ,
            print ' '.join(cmd_output("ls -dl '%s'" % self.name).split()[:8]) ,
            nlines = cmd_output("wc -l '%s'" % self.name)[1:].split() or ['?']
...
class FileComments(dict):
    ...
    def uncommented_files_summary(self):
        ...
        while ucf:  # build lines of uncommented file names
            fn = ucf.pop(0)
            if fn:
                xline = '%s %s' % (line, repr(fn)[1:-1])  # append name
                if len(xline) < FileComment.cols - 4:  # fits
                    line = xline
                else:  # start new line
    ...
    def commented_ones(self):
        ...
        return sorted(self[x].repname for x in self if self[x].comment)

    def selected_ones(self):
        """Return selected subset."""

        return sorted(self[x].repname for x in self if self[x].selected)
